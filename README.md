...
(async () => {
  ...
  const { RholangExtension } = await import(
    "@f1r3fly-io/lightning-bug/extensions"
  );
...
| ------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `getState`         | <table><tr><td>none</td></tr></table>                                                                                                                                           | <table><tr><td>`EditorState`</td></tr></table>                                                                                                                                                                                                                              | Returns the full current state (workspace, diagnostics, symbols, etc.).                                                                                                                                                         | <table><tr><td>`editor.getState();`</td></tr></table>                                                                                                                                                                                                               |
| `getEvents`        | <table><tr><td>none</td></tr></table>                                                                                                                                           | <table><tr><td>`Observable<{ type: string; data: any }>`</td></tr></table>                                                                                                                                                                                                  | Returns RxJS observable for subscribing to events.                                                                                                                                                                              | <table><tr><td>`editor.getEvents().subscribe(event => console.log(event.type, event.data));`</td></tr></table>                                                                                                                                                      |
| `getCursor`        | <table><tr><td>none</td></tr></table>                                                                                                                                           | <table><tr><td>`{ line: number; column: number }`</td></tr></table>                                                                                                                                                                                                         | Returns current cursor position (1-based) for active document.                                                                                                                                                                  | <table><tr><td>`editor.getCursor();`</td></tr></table>                                                                                                                                                                                                              |
...
```javascript
const query =
  "[:find ?text . :where [?a :workspace/active-uri ?uri] [?e :document/uri ?uri] [?e :document/text ?text]]";
...
```javascript
const query =
  "[:find ?text . :in $ ?uri :where [?e :document/uri ?uri] [?e :document/text ?text]]";
...
```javascript
const query =
  "[:find (count ?e) :where [?e :type :document] [?e :document/opened true]]";
...
3. Use ref methods. Access methods via the ref after the editor is ready.
   ...
   const interval = setInterval(() => {
     if (editorRef.current && editorRef.current.isReady()) {
       clearInterval(interval);
       // Subscribe to events
       const sub = editorRef.current
         .getEvents()
         .subscribe((event) => console.log(event.type, event.data));
       // Open a document
       editorRef.current.openDocument(
         "inmemory://test.rho",
         'new x in { x!("Hello") }',
       ...
       // Center on range
       editorRef.current.centerOnRange(
         { line: 2, column: 1 },
         { line: 2, column: 10 }
       );
       // Get text
...
2. Use ref methods. Type-safe access to methods.
   ...
   const interval = setInterval(() => {
     if (editorRef.current && editorRef.current.isReady()) {
       clearInterval(interval);
       const sub: Subscription = editorRef.current
         .getEvents()
