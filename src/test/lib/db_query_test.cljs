...
(deftest documents-returns-all-documents
  (testing "documents returns all documents in database"
    ...
    (h/create-test-document! {:uri "file:///b.rho"
                              :text "b"
                              :language "rholang"})
    (let [docs (db/documents (ws/default-conn))]
      (is (= 2 (count docs)))
...
(deftest document-text-by-uri-returns-text
  (testing "document-text-by-uri returns document text"
    (h/create-test-document! {:uri "file:///test.rho"
                              :text "test content"})
    (is (= "test content" (db/document-text-by-uri (ws/default-conn) "file:///test.rho")))))

(deftest document-language-by-uri-returns-language
  (testing "document-language-by-uri returns document language"
    (h/create-test-document! {:uri "file:///test.rho"
                              :language "rholang"})
    (is (= "rholang" (db/document-language-by-uri (ws/default-conn) "file:///test.rho")))))
...
(deftest active-uri-text-lang-returns-all
  (testing "active-uri-text-lang returns all active document info"
    ...
    (let [[uri text lang] (db/active-uri-text-lang (ws/default-conn))]
...
(deftest doc-text-version-by-uri-returns-both
  (testing "doc-text-version-by-uri returns text and version"
    (h/create-test-document! {:uri "file:///test.rho"
                              :text "test text"
                              :version 7})
    (let [[text version] (db/doc-text-version-by-uri (ws/default-conn) "file:///test.rho")]
      (is (= "test text" text))
      (is (= 7 version)))))
...
(deftest doc-id-text-lang-by-uri-returns-all
  (testing "doc-id-text-lang-by-uri returns id, text, and language"
    (let [id (h/create-test-document! {:uri "file:///test.rho"
                                       :text "test text"
                                       :language "rholang"})
          [returned-id text lang] (db/doc-id-text-lang-by-uri (ws/default-conn) "file:///test.rho")]
      (is (= id returned-id))
      (is (= "test text" text))
      (is (= "rholang" lang)))))
...
(deftest document-language-opened-by-uri-returns-both
  (testing "document-language-opened-by-uri returns language and opened status"
    (h/create-test-document! {:uri "file:///test.rho"
                              :language "rholang"
                              :opened true})
    (let [[lang opened] (db/document-language-opened-by-uri (ws/default-conn) "file:///test.rho")]
      (is (= "rholang" lang))
      (is (true? opened)))))
...
(deftest inc-document-version-by-uri!-non-existent-uri
  (testing "Incrementing version for non-existent URI throws due to nil entity id"
    ...
    (is (thrown-with-msg? js/Error
                          ...
                          (db/inc-document-version-by-uri! (ws/default-conn) "file:///does-not-exist.rho")))))
...
(deftest inc-document-version-by-id!-invalid-id
  (testing "Incrementing version for invalid ID handles gracefully"
    ...
    (let [result (db/inc-document-version-by-id! (ws/default-conn) nil)]
...
(deftest update-document-text-by-uri!-non-existent-uri
  (testing "Updating text for non-existent URI does not throw"
    ...
    (db/update-document-text-by-uri! (ws/default-conn) "file:///does-not-exist.rho" "new text")
    (is (nil? (db/document-text-by-uri (ws/default-conn) "file:///does-not-exist.rho")))))
...
(deftest delete-document-by-id!-cascades-symbols-and-diagnostics
  (testing "Deleting document cascades to symbols and diagnostics"
    (let [id (h/create-test-document! {:uri "file:///cascade.rho"})]
      ;; Add diagnostics
      (db/replace-diagnostics-by-uri! (ws/default-conn) "file:///cascade.rho" nil
                                      [{:message "error"
                                        :severity 1
                                        :startLine 0
                                        :startChar 0
                                        ...
                                        :endChar 5}])
      ;; Add symbols
      (db/replace-symbols! (ws/default-conn) "file:///cascade.rho"
                           (db/flatten-symbols
                            [{:name "func"
                              :kind 12
                              :range {:start {:line 0 :character 0}
      ...
      ;; Verify they exist
      (is (= 1 (count (db/diagnostics-by-uri (ws/default-conn) "file:///cascade.rho"))))
      (is (= 1 (count (db/symbols-by-uri (ws/default-conn) "file:///cascade.rho"))))
      ;; Delete document
      (db/delete-document-by-id! (ws/default-conn) id)
      ;; Both should be gone due to DataScript ref cascading
      (is (empty? (db/diagnostics-by-uri (ws/default-conn) "file:///cascade.rho")))
      (is (empty? (db/symbols-by-uri (ws/default-conn) "file:///cascade.rho"))))))
...
(deftest replace-diagnostics-by-uri!-filters-stale-versions
  (testing "Diagnostics are filtered when version doesn't match document version"
    ...
    (db/replace-diagnostics-by-uri! (ws/default-conn) "file:///versioned.rho" 5
                                    [{:message "current error"
                                      ...
                                      :endLine 0
                                      :endChar 5}])
    (is (= 1 (count (db/diagnostics-by-uri (ws/default-conn) "file:///versioned.rho"))))
    ;; Increment document version
    (db/inc-document-version-by-uri! (ws/default-conn) "file:///versioned.rho")
    ;; Now version is 6, but diagnostics are at version 5
    ;; When querying, the version mismatch should filter them
    (let [diags (db/diagnostics-by-uri (ws/default-conn) "file:///versioned.rho")]
...
(deftest empty-document-text-handling
  (testing "Documents can have empty text"
    (h/create-test-document! {:uri "file:///empty.rho"
                              :text ""})
    (is (= "" (db/document-text-by-uri (ws/default-conn) "file:///empty.rho")))))
  ...
  (testing "Unicode characters in document text are preserved"
    (let [unicode-text "// 你好世界 Hello 🌍\nλx.x → identity\nΩ = ω ω\n// Привет мир"]
      (h/create-test-document! {:uri "file:///unicode.rho"
                                :text unicode-text})
      (is (= unicode-text (db/document-text-by-uri (ws/default-conn) "file:///unicode.rho"))))))
...
(deftest document-dirty-flag-transitions
  (testing "Dirty flag transitions correctly through operations"
    (let [id (h/create-test-document! {:uri "file:///dirty-transitions.rho"
      ...
      (db/update-document-text-by-uri! (ws/default-conn) "file:///dirty-transitions.rho" "modified")
      (is (true? (db/document-dirty-by-uri (ws/default-conn) "file:///dirty-transitions.rho")))
      ;; Save should clear dirty flag
      (db/document-saved-by-uri! (ws/default-conn) "file:///dirty-transitions.rho")
      (is (false? (db/document-dirty-by-uri (ws/default-conn) "file:///dirty-transitions.rho")))
...
(deftest document-state-transitions-valid-property
  (testing "Document state transitions produce valid states"
    (let [_id (h/create-test-document! {:uri "file:///state-test.rho"
                                       ...
                                       :opened false})]
      ;; Open the document
      (db/document-opened-by-uri! (ws/default-conn) "file:///state-test.rho")
      (is (true? (db/document-opened-by-uri? (ws/default-conn) "file:///state-test.rho")))
      ;; Update text - should set dirty
      (db/update-document-text-by-uri! (ws/default-conn) "file:///state-test.rho" "modified")
      (is (true? (db/document-dirty-by-uri (ws/default-conn) "file:///state-test.rho")))
      ;; Increment version
      (db/inc-document-version-by-uri! (ws/default-conn) "file:///state-test.rho")
      (is (= 1 (second (db/document-id-version-by-uri (ws/default-conn) "file:///state-test.rho"))))
      ;; Close the document
      (db/document-closed-by-uri! (ws/default-conn) "file:///state-test.rho")
      (is (false? (db/document-opened-by-uri? (ws/default-conn) "file:///state-test.rho")))
      ;; Document should still exist with correct state
      (let [[text lang dirty] (db/doc-text-lang-dirty-by-uri (ws/default-conn) "file:///state-test.rho")]
        (is (= "modified" text))
        (is (= "rholang" lang))
        (is (true? dirty))))))
...
(deftest document-version-monotonic-property
  (testing "Document version always increases"
    ...
    (let [versions (atom [0])]
      (dotimes [_ 10]
        (let [new-version (db/inc-document-version-by-uri! (ws/default-conn) "file:///monotonic.rho")]
...
(deftest coalesced-queries-return-consistent-results
  (testing "EXP-007 coalesced queries return consistent results with individual queries"
    (h/create-test-document! {:uri "file:///coalesced.rho"
                              ...
                              :dirty true
                              :opened true})
    (db/update-active-uri! (ws/default-conn) "file:///coalesced.rho")
    ;; Test active-uri-text-lang vs individual queries
    (let [[uri text lang] (db/active-uri-text-lang (ws/default-conn))]
      (is (= (db/active-uri (ws/default-conn)) uri))
      (is (= (db/active-text (ws/default-conn)) text))
      (is (= (db/active-lang (ws/default-conn)) lang)))
    ;; Test active-uri-text-lang-version
    (let [[uri text lang version] (db/active-uri-text-lang-version (ws/default-conn))]
      (is (= (db/active-uri (ws/default-conn)) uri))
      (is (= (db/active-text (ws/default-conn)) text))
      (is (= (db/active-lang (ws/default-conn)) lang))
      (is (= (db/active-version (ws/default-conn)) version)))
    ;; Test doc-text-lang-version-by-uri
    (let [[text lang version] (db/doc-text-lang-version-by-uri (ws/default-conn) "file:///coalesced.rho")]
      (is (= (db/document-text-by-uri (ws/default-conn) "file:///coalesced.rho") text))
      (is (= (db/document-language-by-uri (ws/default-conn) "file:///coalesced.rho") lang))
      (is (= (db/document-version-by-uri (ws/default-conn) "file:///coalesced.rho") version)))))
...
(deftest diagnostics-with-nil-version-always-match
  (testing "Diagnostics without version always match document version"
    ...
    (db/replace-diagnostics-by-uri! (ws/default-conn) "file:///nil-version.rho" nil
                                    [{:message "no version diagnostic"
                                      ...
                                      :endChar 5}])
    ;; Should be visible
    (is (= 1 (count (db/diagnostics-by-uri (ws/default-conn) "file:///nil-version.rho"))))
    ;; Increment version multiple times
    (db/inc-document-version-by-uri! (ws/default-conn) "file:///nil-version.rho")
    (db/inc-document-version-by-uri! (ws/default-conn) "file:///nil-version.rho")
    (db/inc-document-version-by-uri! (ws/default-conn) "file:///nil-version.rho")
