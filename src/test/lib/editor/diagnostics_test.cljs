...
(deftest build-decorations-with-diags
  (async done
         (go
           (let [doc "test content"
                 diags [#js {:startLine 0 :startChar 0 :endLine 0 :endChar 4 :severity 1}
                        #js {:startLine 0 :startChar 5 :endLine 0 :endChar 7 :severity 2}]
                 state (.create EditorState #js {:doc doc :extensions #js [diagnostics/diagnostic-field]})
                 view (EditorView. #js {:state state :parent js/document.body})
...
(deftest plugin-updates-on-doc-change
  (async done
         (go
           (let [doc "initial"
                 diags [#js {:startLine 0 :startChar 0 :endLine 0 :endChar 3 :severity 1}]
             ...
             (let [updated-decos (.-decorations plugin-instance)]
               ...
               ;; Check adjusted range (now from 8 to 11, but diag diag is flat, LSP would resend.
               )
             (.destroy view)
             (done)))))
...
(deftest build-decorations-invalid-diag
  (async done
         (go
           (let [doc "test content"
                 invalid-diags [#js {:severity 1} ;; no range attrs
                                #js {:startLine nil :endLine 0 :endChar 4 :severity 2}
                                #js {:startLine 0 :startChar 5 :endLine nil :severity 3}
                                #js {:startLine 0 :startChar 0 :endLine 0 :endChar -1 :severity 4}]
                 state (.create EditorState #js {:doc doc :extensions #js [diagnostics/diagnostic-field]})
                 view (EditorView. #js {:state state :parent js/document.body})
