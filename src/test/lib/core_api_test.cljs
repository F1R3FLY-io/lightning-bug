(ns test.lib.core-api-test
  (:require
   ...
   [test.lib.utils :refer [wait-for wait-for-ready wait-for-event wait-for-uri wait-for-opened-uri
                           ref->editor editor->save-document!
                           editor->open-document! editor->cursor
                           editor->set-cursor! editor->clear-highlight! editor->highlight-range!
                           editor->set-selection! editor->set-text!
                           editor->center-on-range! editor->text editor->events editor->state
                           editor->file-uri editor->diagnostics editor->symbols
                           editor->db editor->search-term editor->log-level editor->set-log-level!
                           editor->query]]
   [test.lib.core-test-common :refer [mount-component wrap-flush cleanup-container once-fixtures each-fixtures]]))
